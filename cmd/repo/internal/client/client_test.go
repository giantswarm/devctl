...
func TestPrintRecord(t *testing.T) {
	...
	PrintRecord(&out, &manager.Record{
		...
		CircleCI:    &manager.CircleCIFacts{Followed: &followed, Head: &manager.Statuses{State: "success"}, Source: "statuses+artifact"},
	...
	for _, want := range []string{
		...
		"circleci: followed yes, setup workflows unknown, head success, from statuses+artifact",
