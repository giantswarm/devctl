...
func (r *runner) run(ctx context.Context, arg string) error {
	...
	args := map[string]any{"repository": repository, "toTeam": r.flag.ToTeam}
	return microerror.Mask(write.Run(ctx, session, r.stdout, &r.flag.Flags, manager.ToolTransferRepository, args))
}
