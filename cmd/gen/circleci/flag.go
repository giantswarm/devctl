...
func (f *flag) Init(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&f.GoBuildPath, flagGoBuildPath, "", `Override the package the go-build job compiles (architect go-build "path" param). Empty keeps the orb default "." (the module root). Set it for Go repos whose module root is a library and whose server main lives in a subdirectory (e.g. coredns-warnlist-plugin -> ./cmd/coredns): building "." there yields a Go archive, not an executable. The append-only custom.yml merge cannot change a generated job's params. The orb also derives its -X .../pkg/project.{gitSHA,buildTimestamp} ldflags from this path, so a repo whose pkg/project sits at the module root loses that stamping when it is set. Only applies with --language=go.`)
