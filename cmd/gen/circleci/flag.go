...
func (f *flag) Init(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&f.KeepChartAppVersion, flagKeepChartAppVersion, false, "Leave the appVersion declared in Chart.yaml alone (push-to-app-catalog `override_app_version: false`). By default app-build-suite stamps appVersion with the computed build version, so the published chart advertises the chart's own version. Set it for a chart that vendors an upstream release and declares that upstream version as its appVersion. The append-only custom.yml merge cannot add a param to a generated job.")
