...
func (f *flag) Init(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&f.ImagePreBuildJob, flagImagePreBuildJob, "", "Name of a repo-owned job (defined in .circleci/custom.yml) the image build must wait on. Adds a `requires` entry to every generated build-image job, which the append-only custom.yml merge cannot inject into a generated job. Used for workspace-handoff pre-steps. Empty for the common case.")
	...
	cmd.Flags().StringVar(&f.ImagePlatforms, flagImagePlatforms, "", "Override the architectures the image is built for. Emits one architect/build-image job per platform, each pinned to a resource class of that platform's architecture, and sets the `platforms` param the push-to-registries merge expects. Empty means linux/amd64,linux/arm64. Set it for single-architecture images (e.g. vllm -> linux/arm64, whose amd64 build has no prebuilt wheels). A platform with no mapped resource class is rejected, since defaulting it would emit a silently emulated build.")
