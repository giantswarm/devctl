...
func writeCommonFiles(dir string, t Template, s substitutions) error {
	codeowners := fmt.Sprintf("# generated by giantswarm/github actions - changes will be overwritten\n* @giantswarm/%s\n", s.Team)
	if err := writeFile(filepath.Join(dir, "CODEOWNERS"), []byte(codeowners), fileMode); err != nil {
		return microerror.Mask(err)
	}
