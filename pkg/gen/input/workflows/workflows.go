...
// release-please cleanup helpers. The release-please flow is no longer
// generated by devctl; these inputs delete stale files that prior gen runs
// left in repos that had been opted into `releaseWorkflow: release-please`.
// Remove once every affected repo has been swept (see release_please_deletions.go).

func (w *Workflows) ReleasePleaseDeletion() input.Input {
	return file.NewReleasePleaseDeletionInput(w.params)
}

func (w *Workflows) ReleasePleaseConfigDeletion() input.Input {
	return file.NewReleasePleaseConfigDeletionInput()
}

func (w *Workflows) ReleasePleaseManifestDeletion() input.Input {
	return file.NewReleasePleaseManifestDeletionInput()
}
