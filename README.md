...
#### Release Please and conventional commits

`devctl gen workflows --release-workflow=release-please` writes a Release Please
workflow plus a `release-please-config.json`. The config maps Conventional
Commit types to [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
section headers:
...
| `fix:`                          | `### Fixed`       |
| `security:`                     | `### Security`    |
| `perf:`, `refactor:`, `docs:`, `style:`, `test:`, `build:`, `ci:`, `chore:`, `revert:` | `### Changed` |

`security:` (or `security(scope):`) is a Giant Swarm extension to the Angular
preset for any PR that ships a CVE fix or vulnerability mitigation. Release
Please routes it to `### Security` in the generated CHANGELOG. The custom type
is required because Release Please's `changelog-sections` matches on the
commit type only, so `fix(security):` cannot be routed to a non-`Fixed`
section.

`--changelog-style=legacy` (the default) keeps the same KaC mapping while
omitting commit types not historically generated by `devctl` users (`style`,
`revert`). `--changelog-style=release-please` enables the full set.
