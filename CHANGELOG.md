...
- `repo setup`: default `--checks-filter` now excludes `validate-changelog` and `check-values-schema` in addition to `aliyun`. Both are path-conditional generated workflows (`validate-changelog` only runs when `CHANGELOG.md` changes; `check-values-schema` is managed explicitly by the align-files cycle). Auto-pinning them as required status checks blocks PRs that don't touch the filtered paths.
