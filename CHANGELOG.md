...
- `gen workflows` (`--release-workflow=release-please`): rewrite legacy `## [X.Y.Z] - YYYY-MM-DD` headers in `CHANGELOG.md` into the inline-link form release-please emits for new releases: `## [X.Y.Z](https://<host>/<owner>/<repo>/compare/vPREV...vX.Y.Z) (YYYY-MM-DD)`. PREV is the version of the next `## [V.W.Z]` heading below it in the file (CHANGELOGs are descending). The oldest version falls back to a release-tag link. Idempotent — already-linked headers don't match the legacy regex. No-op when `CHANGELOG.md` doesn't exist, contains no legacy headers, or the `origin` remote URL can't be parsed (SSH shorthand `git@host:owner/repo`, `ssh://[user@]host/owner/repo` — which an `insteadOf` config produces from an HTTPS remote — and `https://host/owner/repo` are all recognised, plus the `.git` suffix is optional on all three).
