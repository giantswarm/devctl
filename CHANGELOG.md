...
- `gen workflows`: remove `paths:` filters from the generated `validate-changelog` and `check-values-schema` calling workflows. With the filters in place, PRs that do not touch `CHANGELOG.md` or Helm values files never triggered those workflows, leaving the required status checks permanently unsatisfied and blocking all merges. Without the filters both workflows fire on every PR; the `validate-changelog` job is still gated by its reusable-workflow `if:` condition (skipped for non-release branches, which GitHub treats as passing), and `check-values-schema` iterates over `helm/*/Chart.yaml` globs that evaluate to empty on non-chart PRs.
